package test.US_042_T;

import org.testng.Assert;
import org.testng.annotations.Test;
import pages.US_042_P.US_042_3_P;
import utilities.ConfigReader;
import utilities.Driver;
import utilities.TestBaseReports;

public class TC_42_3_T extends TestBaseReports {


    US_042_3_P pickPage = new US_042_3_P();// objemizi oluşturduk
    ...
    public void test1() throws InterruptedException {
      ...
      // Assert.assertTrue(pickPage.stokDurumuButton.isDisplayed());
        ...
        //kullanıcı addtocahart yapar
        pickPage.addToChartButton.click();


        // kullanıcı stok sayısından fazla ürün girişi yapamamasının doğrulanması

        String expectednumber =pickPage.availableButton.getText();

        int expectedNO = Integer.parseInt(expectednumber.substring(0,1));

        for (int i = 0; i <= expectedNO+1 ; i++) {

            pickPage.artiPlusButton.click();
        }

       int chartNumber = Integer.parseInt(pickPage.sayiAddToChart.getText());

        Assert.assertEquals(expectedNO,chartNumber);
